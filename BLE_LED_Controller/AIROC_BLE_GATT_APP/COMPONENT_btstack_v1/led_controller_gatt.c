...
wiced_bt_gatt_status_t led_controller_gatts_req_cb( wiced_bt_gatt_attribute_request_t *p_req );
wiced_bt_gatt_status_t led_controller_gatts_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data);
wiced_bt_gatt_status_t led_controller_gatt_op_comp_cb( wiced_bt_gatt_operation_complete_t *p_data );

/* Extern variables */
extern led_controller_state_t led_controller_state;
extern host_info_t led_controller_hostinfo;
...
wiced_bt_gatt_status_t led_controller_gatts_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data)
{
    ...
    case GATT_CONNECTION_STATUS_EVT:
        result = led_controller_gatts_conn_status_cb( &p_data->connection_status );
        break;

    case GATT_ATTRIBUTE_REQUEST_EVT:
        result = led_controller_gatts_req_cb( &p_data->attribute_request );
        break;

    case GATT_OPERATION_CPLT_EVT:
        result = led_controller_gatt_op_comp_cb( &p_data->operation_complete );
        break;
 ...
 */
wiced_bt_gatt_status_t led_controller_gatts_req_read_handler( uint16_t conn_id, wiced_bt_gatt_read_t * p_read_data )
{
    attribute_t *puAttribute;
    int          attr_len_to_copy;

    if ( ( puAttribute = led_controller_get_attribute(p_read_data->handle) ) == NULL)
    {
 ...
 */
wiced_bt_gatt_status_t led_controller_gatts_req_write_exec_handler( uint16_t conn_id, wiced_bt_gatt_exec_flag_t exec_falg )
{
 ...
 */
wiced_bt_gatt_status_t led_controller_gatts_req_mtu_handler( uint16_t conn_id, uint16_t mtu)
{
...
wiced_bt_gatt_status_t led_controller_gatts_req_cb( wiced_bt_gatt_attribute_request_t *p_req )
{
    ...
    WICED_BT_TRACE( "led_controller_gatts_req_cb. conn %d, type %d\n", p_req->conn_id, p_req->request_type );

    switch ( p_req->request_type )
    {
    case GATTS_REQ_TYPE_READ:
        result = led_controller_gatts_req_read_handler( p_req->conn_id, &(p_req->data.read_req) );
        break;

    case GATTS_REQ_TYPE_WRITE:
        result = led_controller_gatts_req_write_handler( p_req->conn_id, &(p_req->data.write_req) );
        break;

    case GATTS_REQ_TYPE_WRITE_EXEC:
        result = led_controller_gatts_req_write_exec_handler( p_req->conn_id, p_req->data.exec_write );
        break;

    case GATTS_REQ_TYPE_MTU:
        result = led_controller_gatts_req_mtu_handler( p_req->conn_id, p_req->data.mtu );
        break;
   default:
        break;
    }
